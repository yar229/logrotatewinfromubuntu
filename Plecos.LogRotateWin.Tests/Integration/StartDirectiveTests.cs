...
";
            ...
            try
            {
                ...
                // Assert - With rotate 3, should keep up to 3 rotated files (.0 through .2)
                File.Exists($"{logFile}.3").Should().BeFalse("fourth rotation should not create .3 file");
                File.Exists($"{logFile}.2").Should().BeTrue("third rotation should create .2 file");
                File.Exists($"{logFile}.1").Should().BeTrue(".1 file should still exist");
                File.Exists($"{logFile}.0").Should().BeTrue(".0 file should still exist with rotate count 2");
...
";
        ...
        [Fact]
        public void RotateLog_WithStart2_ShouldStartAt2()
        {
            // 'start 0' directive causes rotated files to start at .0 instead of .1
            // This is useful for compatibility with some applications that expect .0 as first rotation
...
"; // cannot use 0 for start
            ...
            try
            {
                ...
                // Assert - First rotated file should be .0, not .1
                File.Exists($"{logFile}.2").Should().BeTrue("first rotation should create .0 file when start is 0");
                File.Exists($"{logFile}.3").Should().BeFalse(".3 should not exist when start is 2");
                File.Exists(logFile).Should().BeTrue("original log file should be recreated");
                ...
                // Assert - Should now have .0 and .1
                File.Exists($"{logFile}.3").Should().BeTrue("second rotation should create .1 file");
                File.Exists($"{logFile}.2").Should().BeTrue(".0 file should still exist");
                ...
                // Assert - Should now have .0, .1, and .2
                File.Exists($"{logFile}.4").Should().BeTrue("third rotation should create .2 file");
                File.Exists($"{logFile}.3").Should().BeTrue(".1 file should still exist");
                File.Exists($"{logFile}.2").Should().BeTrue(".0 file should still exist");
            }
            finally
            {
...
";
        ...
        [Fact]
        public void RotateLog_WithStart2AndRotateLimit_ShouldKeepCorrectCount()
        {
            // Tests that start 0 works correctly with rotate count limits
            // The rotate directive specifies how many rotated logs to keep
...
";
            ...
            try
            {
                // Act - First rotation
                RunLogRotate("-s", stateFile, "-f", configFile);
                File.Exists($"{logFile}.2").Should().BeTrue("first rotation should create .2");

                // Act - Second rotation
                File.WriteAllText(logFile, "New log content\n");
                RunLogRotate("-s", stateFile, "-f", configFile);
                File.Exists($"{logFile}.3").Should().BeTrue(".3 should be created");
                File.Exists($"{logFile}.2").Should().BeTrue(".2 should still exist");

                // Act - Third rotation
                File.WriteAllText(logFile, "Another log content\n");
                RunLogRotate("-s", stateFile, "-f", configFile);

                // Assert - With rotate 2, should keep up to 2 rotated files (.2 through .4)
                File.Exists($"{logFile}.4").Should().BeTrue("third rotation should create .4 file");
                File.Exists($"{logFile}.3").Should().BeTrue(".3 file should still exist");
                File.Exists($"{logFile}.2").Should().BeTrue(".2 file should still exist with rotate count 2");
            }
            finally
            {
                TestHelpers.CleanupPath(configFile);
...
";
        ...
        [Fact]
        public void RotateLog_WithStart2AndCompress_ShouldWorkTogether()
        {
            // Tests that start 2 works correctly with compression
            // Compressed files should also follow the start numbering
...
";
            ...
            try
            {
                ...
                // Assert - Compressed file should be .0.gz
                File.Exists($"{logFile}.2.gz").Should().BeTrue("first rotation should create .2.gz file when start is 2");
                File.Exists($"{logFile}.3.gz").Should().BeFalse(".3.gz should not exist when start is 2");
                ...
                // Assert - Should have .0.gz and .1.gz
                File.Exists($"{logFile}.3.gz").Should().BeTrue("second rotation should create .3.gz file");
                File.Exists($"{logFile}.2.gz").Should().BeTrue(".2.gz file should still exist");
            }
            finally
            {
                TestHelpers.CleanupPath(configFile);
